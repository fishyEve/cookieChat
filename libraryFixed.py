...
def sendingPromptMessage(promptMessage):

	#converts the prompt text into a python dictionary
	promptDictionary = {'Type': 'prompt', 'Prompt': promptMessage}

	#converts the dictionary into a json string
	promptJString = json.dumps(promptDictionary)
...
def totallyReadingText(yourSocket):
	...
	#pulls out the necessary fields to be printed in the chat
	if (textDictionary['Type'] == 'prompt'):
		textString = textDictionary['Prompt']
	elif(textDictionary['Type'] == 'chat'):
		textString = textDictionary['Timestamp'] + " " + textDictionary['User'] + ": " + textDictionary['Message']
...
#Function Name: totallyBytes
#Description: This function allows the proper amount of bytes to be read- JUST BYTES- this does not turn it into a string
#				Kenneth (server) can use this function when sending messages to other clients
#				For example, server will receive the message in the form of bytes using totallyBytes(yourSocket) and will then
#				send those bytes over to other clients using sendall()
#				Possible use case: chatMsg = totallyBytes(yourSocket)
#									mySocket.sendall(chatMsg)
#Return Value: textBytes- THESE ARE THE BYTES THAT WERE SENT
def totallyBytes(yourSocket):
	
	#reads in the length of the sent message in bytes
	textLength = int.from_bytes((totallyReadingBytes(yourSocket, 2)), 'big')

	#Receives the actual bytes that need to be read/sent over
	textBytes = totallyReadingBytes(yourSocket, textLength)

	#return statement
	return textBytes
	#end of totallyBytes function
...
def sendingChats(nickname, message):
	...
	messageDict = {'Type': 'chat', 'Timestamp': strTimeStamp, 'User': nickname, 'Message': message}
